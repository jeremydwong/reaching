...
f = opti.parameter()
opti.set_value(f,  f_dim*tnd)  # frequency
Vo = opti.parameter()
opti.set_value(Vo, Vo_dim*tnd/L)  # speed at initial contact
...
def qd(q, u): return vertcat(q[1], q[2]-1, q[3], u[0])  # dq/dt = f(q,u)
...
def Int(pP, qP, pFdd, qFdd): return pP+qP + pFdd+qFdd + pP * \
    qP + pFdd*qFdd  # J = |power| + |Fdd| & complementarity
